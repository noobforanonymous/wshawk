...
class WebhookNotifier:
    ...
    def _build_slack_payload(self, vulns: List[Dict], scan_info: Dict) -> Dict:
        ...
        blocks.append({
            "type": "context",
            "elements": [
                {"type": "mrkdwn", "text": f"_WSHawk v2.0.7 by Regaan | {datetime.now().strftime('%Y-%m-%d %H:%M')}_ "}
            ]
        })
    ...
    def _build_discord_payload(self, vulns: List[Dict], scan_info: Dict) -> Dict:
        ...
        embed = {
            ...
            "fields": fields,
            "footer": {
                "text": f"WSHawk v2.0.7 by Regaan • {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            },
            "thumbnail": {
